local BilinearSamplerBDHW, parent = torch.class('nn.BilinearSamplerBDHW', 'nn.Module')

--[[
   BilinearSamplerBDHW() :
   BilinearSamplerBDHW:updateOutput({inputImages, grids})
   BilinearSamplerBDHW:updateGradInput({inputImages, grids}, gradOutput)

   BilinearSamplerBDHW will perform bilinear sampling of the input images according to the
   ...
   - inputImages has to be in BDHW layout

   - grids have to be in BDHW layout, with dim(D)=2
...
function BilinearSamplerBDHW:updateOutput(input)
   ...
   self.output:resize(inputImages:size(1), inputImages:size(2), grids:size(3), grids:size(4))
   
   self.output:contiguous()

   inputImages.nn.BilinearSamplerBDHW_updateOutput(self, inputImages, grids, self.output)
...
function BilinearSamplerBDHW:updateGradInput(_input, _gradOutput)
   ...
   inputImages.nn.BilinearSamplerBDHW_updateGradInput(self, inputImages, grids, gradInputImages, gradGrids, gradOutput)
